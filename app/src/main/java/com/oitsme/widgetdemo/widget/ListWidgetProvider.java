...
public class ListWidgetProvider extends AppWidgetProvider {
    ...
    public void onUpdate(Context context, AppWidgetManager appWidgetManager,
        ...
        LogU.showElog("ListWidgetProvider onUpdate");
        for (int appWidgetId : appWidgetIds) {
            // 获取AppWidget对应的视图
            RemoteViews remoteViews = new RemoteViews(context.getPackageName(), R.layout.layout_widget);

            remoteViews.setTextViewText(R.id.tvShow,num+" 数字显示了");
            ...
            Intent listIntent = new Intent(context, ListWidgetProvider.class);
            listIntent.setAction(COLLECTION_VIEW_ACTION);
            listIntent.putExtra(AppWidgetManager.EXTRA_APPWIDGET_ID, appWidgetId);
            PendingIntent pendingIntent = PendingIntent
                .getBroadcast(context, appWidgetId, listIntent, PendingIntent.FLAG_UPDATE_CURRENT);
            // 设置intent模板
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (context != null) {
            ...
            LogU.showElog("  ssss  " + action);
            AppWidgetManager appWidgetManager = AppWidgetManager.getInstance(context);
            if (TextUtils.equals(action, COLLECTION_VIEW_ACTION)) {
            ...
            } else if (TextUtils.equals(action, REFRESH_WIDGET)) {
